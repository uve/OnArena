...
def defer(obj, *args, **kwargs):
  ...
  if memcache.get(key_name):
    logging.info("deferred get: True")  
    return True
    
                      
  if memcache.set(key_name, True):
    logging.info("deferred set: True")      
  else:
    logging.info("deferred set: False")      
    
  '''  
    
  
  try:
    #logging.info("pickled: %s",pickled)
    #logging.info("taskargs: %s",taskargs)
